...
const STAGE_TEXTS = {
    ...
    calcStep1: "根据规则，会先减去它的个位数",
    calcStep2: "然后再减去十位上的数字",
    calcStep3: "这就是运算后的结果啦",
    //start: "先输入一个10到19之间的两位数，看看会得到什么结果。",
    firstLight: "读心术的奥秘就藏在点亮的数字里，你可以多输入一些数字，找找它们的数学规律。",
    secondLight: "读心术的奥秘就藏在点亮的数字里，你可以多输入一些数字，找找它们的数学规律。",
...
class AudioManager {
    constructor() {
        ...
        if (this.bgm) this.bgm.volume = 0.14;
    }
...
class UIManager {
    constructor(audio, tts, game) {
        ...
        this.els = {
            ...
            recordModeToggle: document.getElementById('recordModeToggle'),
    ...
    guideInputForNextTry(message = STAGE_TEXTS.inputGuideDefault) {
        ...
        if (this.game.lightRecord.length === 0) {
            ...
            if (this.els.randomBtn) this.els.randomBtn.style.display = 'none';
        } else {
            this.els.inputTen.readOnly = false;
            this.els.cellTen.classList.remove('locked');
            this.els.inputTen.focus();
    ...
    prepareNextRecordBlock() {
        if (this.game.recordMode === 'multi') return;

        const list = this.els.calcRecordList;
        if (!list) return;

        if (this.currentPendingBlock && !this.currentPendingBlock.hasCalculated) return;

        const count = list.children.length + 1;
        const block = document.createElement('div');
        block.className = 'record-block pending-block';
        block.innerHTML = `<div class="record-title">第${count}次输入:</div>`;
        ...
        this.currentPendingBlock = block;
    }

    async updateCalcAnimated(a, b, s) {
        const c1 = a - b;
        const c2 = c1 - s;
        ...
        let block = this.currentPendingBlock;
        if (!block || block.hasCalculated) {
            const count = list.children.length + 1;
        ...
        block.hasCalculated = true;
        block.classList.remove('pending-block');

        // Wait a tick to ensure container flex bounds are updated
        await Utils.wait(10);

        const area = list.parentElement;

        // Immediately scroll to the new block so the user can watch the calculation process
        area.scrollTo({ top: block.offsetTop, behavior: 'smooth' });

        block.insertAdjacentHTML('beforeend', `
            <div class="record-step step-1" style="opacity: 0; transform: translateY(5px); transition: all 0.3s ease;"><span class="text-red">${a}</span> - ${b} = ${c1}</div>
            <div class="record-step step-2" style="opacity: 0; transform: translateY(5px); transition: all 0.3s ease;">${c1} - ${s} = <span class="text-red current-result-num">${c2}</span></div>
            <div class="record-summary step-3" style="opacity: 0; transform: translateY(5px); transition: all 0.3s ease;">输入 <span class="text-red">${a}</span>，运算结果为 <span class="text-red">${c2}</span></div>
        `);

        const title = block.querySelector('.record-title');
        const step1 = block.querySelector('.step-1');
        const step2 = block.querySelector('.step-2');
        const step3 = block.querySelector('.step-3');

        console.log('当前是否是引导模式:', this.isFirstTimeGuide);

        if (this.isFirstTimeGuide) {
            this.guideStep = 2;
            this.clearGuideHighlights();
            const wrapper = this.els.calcRecordList.closest('.calc-record-wrapper');
            if (wrapper) wrapper.classList.add('guide-highlight', 'no-bg');
            ...
            this.showTip(STAGE_TEXTS.calcStep3);

            const prevCurrent = list.querySelectorAll('.current-result-num');
            prevCurrent.forEach((el, index) => {
                if (index < prevCurrent.length - 1) el.classList.remove('current-result-num');
            });
            area.scrollTo({ top: block.offsetTop, behavior: 'smooth' });
            ...
            //一步一行模式下算出结果后飞到格子的动画延时
            await Utils.wait(600);
        }
    }

    async updateCalcAnimatedMulti(num, ge, shi, res) {
        const c1 = num - ge;
        const c2 = c1 - shi;
        ...
        await Utils.wait(this.isFirstTimeGuide ? 300 : 10);
    ...
    lightResultCell(el) {
        ...
        if (this.game.lightRecord.length < 5) {
            el.classList.add('brief-show-bubble');
            setTimeout(() => {
                el.classList.remove('brief-show-bubble');
            }, 2000);
        }
    ...
    revealSecretButton(delay = 0) {
        ...
        this.secretBtnTimer = setTimeout(() => {
            ...
            const anim = this.els.secretBtn.animate([
                { transform: 'scale(1) translateY(0)', boxShadow: '0 4px 10px rgba(217, 119, 6, 0.1)', offset: 0 },
                { transform: 'scale(1.15) translateY(-5px)', boxShadow: '0 0 20px rgba(217, 119, 6, 0.6)', offset: 0.15 },
                { transform: 'scale(1.1) translateY(-3px)', boxShadow: '0 0 15px rgba(217, 119, 6, 0.4)', offset: 0.35 },
                { transform: 'scale(1.15) translateY(-5px)', boxShadow: '0 0 20px rgba(217, 119, 6, 0.6)', offset: 0.55 },
                { transform: 'scale(1.1) translateY(-3px)', boxShadow: '0 0 15px rgba(217, 119, 6, 0.4)', offset: 0.75 },
                { transform: 'scale(1) translateY(0)', boxShadow: '0 4px 10px rgba(217, 119, 6, 0.1)', offset: 1 }
            ], {
                duration: 2500,
                easing: 'ease-in-out'
            });
    ...
    updateTip(num) {
        ...
        if (cnt >= 5) this.els.boardWrap.classList.add('show-bubbles');

        if (cnt === 1) this.showTip(STAGE_TEXTS.firstLight, false);
        else if (cnt === 2) this.showTip(STAGE_TEXTS.secondLight, false);
        else if (cnt === 4) { this.revealSecretButton(1200); this.showTip(STAGE_TEXTS.fourthLight); }
        else if (cnt === 5) { this.revealSecretButton(0); this.showTip(STAGE_TEXTS.fifthLight); }
        else if (cnt === 6) { this.revealSecretButton(0); this.showTip(STAGE_TEXTS.sixthLight); }
        else if (cnt === 7) { this.revealSecretButton(0); this.showTip(STAGE_TEXTS.seventhLight); }
        else if (cnt === 8) { this.revealSecretButton(0); this.showTip(STAGE_TEXTS.eighthLight); }
        else if (cnt >= 9) {
            this.revealSecretButton(0);
            this.showTip(STAGE_TEXTS.ninthLight);
            // 第9个点亮后，语音播完自动模拟点击"发现数学规律"按钮
            setTimeout(() => {
                if (this.els.secretBtn && !this.els.secretBtn.classList.contains('locked')) {
                    this.els.secretBtn.click();
                }
            }, 3500);
        }
    }
    ...
    // --- 事件绑定 ---
    bindEvents() {
        const { inputTen, inputGe, cellTen, cellGe, numpad, numpadClear, numpadDel2, randomBtn, secretBtn, replayBtn, continueExploreBtn, startBtn, recordModeToggle } = this.els;

        if (recordModeToggle) {
            recordModeToggle.addEventListener('change', (e) => {
                if (this.isResultAnimating || this.isFocusAnimating) {
                    e.preventDefault();
                    const activeInput = recordModeToggle.querySelector('.mode-option.active input');
                    if (activeInput) activeInput.checked = true;
                    return;
                }
                if (e.target.name === 'rMode') {
                    this.game.recordMode = e.target.value;
                    recordModeToggle.querySelectorAll('.mode-option').forEach(opt => opt.classList.remove('active'));
                    e.target.closest('.mode-option').classList.add('active');

                    if (this.game.recordMode === 'multi') {
                        this.els.calcRecordList.classList.add('multi-mode');
                    } else {
                        this.els.calcRecordList.classList.remove('multi-mode');
                    }
                    this.reRenderHistory();
                }
            });
            recordModeToggle.addEventListener('click', (e) => {
                if (this.isResultAnimating || this.isFocusAnimating) {
                    e.preventDefault();
                }
            }, true);
        ...
        if (randomBtn) {
            randomBtn.addEventListener('click', () => {
                ...
                let is30Percent = Math.random() < 0.3;
        ...
        secretBtn.addEventListener('click', () => {
            ...
            if (this.els.replayModal) this.els.replayModal.style.display = 'flex';
            this.showTip(STAGE_TEXTS.successSecret);
            if (window.g) window.g.trackComplete();
        });

        replayBtn.addEventListener('click', () => {
            if (this.els.replayModal) this.els.replayModal.style.display = 'none';
            this.playAgain();
        });
    ...
    reRenderHistory() {
        ...
        if (this.game.recordMode === 'multi') {
            ...
            this.game.history.forEach((item, index) => {
                const { num, ge, shi, res } = item;
                const c1 = num - ge;
                const c2 = c1 - shi;
                const count = index + 1;
                ...
                const isLatest = index === this.game.history.length - 1;

                block.innerHTML = `
                    <div class="record-title">第${count}次输入:</div>
                    <div class="record-step"><span class="text-red">${num}</span> - ${ge} = ${c1}</div>
                    <div class="record-step">${c1} - ${shi} = <span class="text-red ${isLatest ? 'current-result-num' : ''}">${c2}</span></div>
                    <div class="record-summary">输入 <span class="text-red">${num}</span>，运算结果为 <span class="text-red">${c2}</span></div>
                `;
                list.appendChild(block);
            });

            if (this.els.inputTen.value && !this.els.inputGe.value && !this.isResultAnimating) {
                this.prepareNextRecordBlock();
            } else if (list.lastElementChild) {
                list.parentElement.scrollTo({ top: list.lastElementChild.offsetTop, behavior: 'smooth' });
            }
        }
    }
    finalizePendingBlock() {
        if (this.currentPendingBlock) {
            this.currentPendingBlock.hasCalculated = true;
            this.currentPendingBlock.classList.remove('pending-block');
        }
    }

    abortPendingBlock() {
        if (this.currentPendingBlock && !this.currentPendingBlock.hasCalculated) {
            this.currentPendingBlock.remove();
            this.currentPendingBlock = null;
        }
    }
    ...
    async executeCalc(num, isRandomAction = false) {
        ...
        if (this.game.recordMode === 'multi') {
            await this.updateCalcAnimatedMulti(num, ge, shi, res);
        } else {
            await this.updateCalcAnimated(num, ge, shi);
        }
        ...
        if (this.game.lastTenDigit !== null && curTen === this.game.lastTenDigit) {
            this.flashLitCell(res);
            const targetTen = this.getNextTenHint(curTen);
            if (isRandomAction) {
                this.showTip(STAGE_TEXTS.sameResultRandom);
            } else if (targetTen !== null) {
                this.showTip(STAGE_TEXTS.sameResultHint(targetTen));
            } else {
                this.showTip(STAGE_TEXTS.sameResult);
            }
            this.game.recordLight(res, curTen);
            this.isResultAnimating = false;
            this.guideInputForNextTry(STAGE_TEXTS.inputGuideSameTen);
            return;
        }
        ...
        if (isAlreadyLight) {
            ...
            const targetTen = this.getNextTenHint(curTen);
            if (isRandomAction) {
                this.showTip(STAGE_TEXTS.alreadyLightRandom);
            } else if (targetTen !== null) {
                this.showTip(STAGE_TEXTS.alreadyLightHint(targetTen));
            } else {
                this.showTip(STAGE_TEXTS.alreadyLight);
            }
            this.game.recordLight(res, curTen);
            this.isResultAnimating = false;
            this.guideInputForNextTry(STAGE_TEXTS.inputGuideAlreadyLit);
            return;
        }

        let isNew = false;
        if (res >= 1 && res <= 99 && !isAlreadyLight) {
            const el = this.numDomList[res - 1];
            this.game.recordLight(res, curTen);
            ...
            if (this.game.lightRecord.length >= 3) {
                ...
                // 等待单次呼吸动画结束 (0.8s) + 额外的延迟 1000 毫秒 = 1800 毫秒
                await Utils.wait(1800);

                // 2. 紧接着，所有已点亮数字共同心跳两次
                this.playFocusAnimation();

                // 共同心跳两次的耗时为 800 * 2 = 1600 毫秒，等待其结束
                await Utils.wait(1600);
            } else {
                await Utils.wait(1500);
            }

            // Both the grid and the hint are already highlighted by .board-panel
        } else {
            this.isResultAnimating = false;
            this.playFocusAnimation();
        }
        ...
        if (this.isFirstTimeGuide) {
            ...
            // 补充 09 的气泡（新手引导期间特意隐藏，现在退出引导需要显示）
